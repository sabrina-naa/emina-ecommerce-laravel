...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->file('foto')) {
            ...
            ImageHelper::uploadAndResize($file, $directory, $originalFileName, 385);
    ...
    $filterRole = $request->input('role'); // null, 0, atau 1
    $filterStatus = $request->input('status'); // null, 0, atau 1
    ...
    $totalSuperAdmin = $user->where('role', 1)->count();
    $totalAdmin = $user->where('role', 0)->count();
    $totalAktif = $user->where('status', 1)->count();
    $totalNonAktif = $user->where('status', 0)->count();
