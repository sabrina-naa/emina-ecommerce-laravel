...
@section('title', 'Promo - Emina Beauty')
...
<div class="bg-gray-50 border-b">
    <div class="container mx-auto px-4 py-3">
        <nav class="flex items-center text-sm" aria-label="Breadcrumb">
            ...
            <span class="text-gray-800 font-medium">Promo</span>
        </nav>
    </div>
</div>

{{-- Promo Banner --}}
<section class="bg-gradient-to-r from-primary to-accent text-white py-8 md:py-12">
    <div class="container mx-auto px-4 text-center">
        <h1 class="text-3xl md:text-5xl font-bold mb-4">
            🎉 Promo Spesial untuk Kamu!
        </h1>
        <p class="text-lg md:text-xl text-white/90 max-w-2xl mx-auto">
            Dapatkan produk kecantikan favoritmu dengan harga terbaik. Jangan sampai kehabisan!
        </p>
    </div>
</section>
        ...
        <div class="flex gap-4">
            ...
            <div class="hidden lg:block w-64 flex-shrink-0">
                <div class="bg-white rounded-lg shadow-sm sticky top-20">
                    ...
                    <form action="{{ route('promo') }}" method="GET" id="filter-form" class="p-4 space-y-4">
                        ...
                        <div class="space-y-2 border-t pt-4">
                            ...
                            <a href="{{ route('promo') }}" class="block w-full text-center border border-gray-300 text-gray-700 py-2 rounded-md font-medium text-sm hover:bg-gray-50 transition">
                                Atur Ulang
                            </a>
                        </div>
                    </form>
                    ...
                    <div class="flex items-center justify-between flex-wrap gap-3">
                        ...
                        <div class="flex items-center gap-2 overflow-x-auto">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('promo', array_merge(request()->except('sort'), ['sort' => 'termurah'])) }}" 
                                   class="px-3 py-1.5 text-sm rounded-md transition {{ request('sort') == 'termurah' || !request('sort') ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                                    Termurah
                                </a>
                                <a href="{{ route('promo', array_merge(request()->except('sort'), ['sort' => 'termahal'])) }}" 
                                ...
                                <a href="{{ route('promo', array_merge(request()->except('sort'), ['sort' => 'terbaru'])) }}" 
                                   class="px-3 py-1.5 text-sm rounded-md transition {{ request('sort') == 'terbaru' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                                    Terbaru
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            <span class="text-sm text-gray-600 whitespace-nowrap">
                                <strong class="text-gray-800">{{ $produk->total() }}</strong> Produk Promo
                            </span>
                        ...
                        <div class="flex items-center gap-2 mt-3 pt-3 border-t flex-wrap">
                            ...
                            @if(request('search'))
                                <span class="inline-flex items-center gap-1 bg-primary/10 text-primary px-2 py-1 rounded-full text-xs">
                                    "{{ request('search') }}"
                                    <a href="{{ route('promo', request()->except('search')) }}" class="hover:text-accent">
                            ...
                            @if(request('kategori'))
                                <span class="inline-flex items-center gap-1 bg-primary/10 text-primary px-2 py-1 rounded-full text-xs">
                                    ...
                                    <a href="{{ route('promo', request()->except('kategori')) }}" class="hover:text-accent">
                                        <i class="fas fa-times"></i>
                                    </a>
                                </span>
                            @endif
                            ...
                            @if(request('harga_min') || request('harga_max'))
                                <span class="inline-flex items-center gap-1 bg-primary/10 text-primary px-2 py-1 rounded-full text-xs">
                                    ...
                                    <a href="{{ route('promo', request()->except(['harga_min', 'harga_max'])) }}" class="hover:text-accent">
                                        <i class="fas fa-times"></i>
                                    </a>
                                </span>
                            @endif
                            
                            <a href="{{ route('promo') }}" class="text-xs text-accent hover:underline ml-2">
                                Hapus Semua
                            </a>
                        </div>
                    @endif
                            ...
                            <div class="bg-white rounded-lg shadow-sm overflow-hidden group hover:shadow-md transition">
                                <a href="{{ route('shop.show', $item->id) }}" class="block">
                                    <div class="relative aspect-square overflow-hidden bg-gray-100">
                                        ...
                                        {{-- Promo Badge --}}
                                        <span class="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded-md font-bold animate-pulse">
                                            PROMO
                                        </span>
                ...
                @else
                    ...
                    <div class="bg-white rounded-lg shadow-sm p-12 text-center">
                        <div class="max-w-md mx-auto">
                            ...
                            <p class="text-gray-500 mb-6">Saat ini belum ada produk promo. Cek kembali nanti ya!</p>
                            <a href="{{ route('shop.index') }}" class="inline-block bg-primary hover:bg-accent text-white px-6 py-2.5 rounded-md font-medium transition">
                                <i class="fas fa-shopping-bag mr-2"></i> Belanja Produk Lainnya
                            </a>
                        </div>
                    </div>
                @endif
...
<div id="mobile-filter-modal" class="fixed inset-0 bg-black/50 z-50 hidden">
    <div class="fixed inset-x-0 bottom-0 bg-white rounded-t-2xl max-h-[80vh] overflow-y-auto">
        ...
        <form action="{{ route('promo') }}" method="GET" class="p-4 space-y-4">
            {{-- Kategori --}}
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Kategori</label>
            ...
            <div class="sticky bottom-0 bg-white pt-4 space-y-2 border-t">
                ...
                <a href="{{ route('promo') }}" class="block w-full text-center border border-gray-300 text-gray-700 py-3 rounded-md font-medium hover:bg-gray-50 transition">
                    Atur Ulang
                </a>
            </div>
        </form>
